...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initScrollFloat() {
        ...
        const scrollFloatContainers = document.querySelectorAll('.scroll-float');
        
        scrollFloatContainers.forEach(container => {
            ...
            const splitTextNodes = (element, nodes) => {
                nodes.forEach(node => {
                    if (node.nodeType === Node.TEXT_NODE) {
                        const text = node.textContent;
                        const words = text.split(/(\s+)/); // Split keeping the spaces
                        
                        words.forEach(word => {
                            if (word === '') return; // Skip empty string from split
                            
                            if (word.trim() === '') {
                                // It's whitespace, create a spacer span so it animate properly and preserves width
                                ...
                                // Wrap word to prevent mid-word breaking
                        ...
                        splitTextNodes(newElement, Array.from(node.childNodes));
                        element.appendChild(newElement);
                    }
                });
            };

            splitTextNodes(textElement, originalNodes);

            const charElements = textElement.querySelectorAll('.char');

            // Apply GSAP FromTo animation mapped to scroll
            gsap.fromTo(
                charElements,
                {
